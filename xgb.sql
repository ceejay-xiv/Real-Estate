...
DROP TABLE nnet_model_crf_settings CASCADE CONSTRAINTS PURGE;

CREATE TABLE nnet_model_crf_settings(
...
BEGIN
    insert into nnet_model_crf_settings VALUES
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL(model_name => 'nnet_model_crf');
END;
/

BEGIN
    DBMS_DATA_MINING.CREATE_MODEL(
    model_name => 'nnet_model_crf',
    mining_function => DBMS_DATA_MINING.classification,
    data_table_name => 'crf_mining_data_build_v',
    case_id_column_name => 'card',
    target_column_name => 'DEFAULTPAYNXTMNT',
    settings_table_name => 'nnet_model_crf_settings');
END;
/
...
SELECT DEFAULTPAYNXTMNT AS Actuals,
    PREDICTION(nnet_model_crf USING*) AS Predicted,
    COUNT(*) AS TOTALS
FROM crf_mining_data_test_v
GROUP BY DEFAULTPAYNXTMNT, PREDICTION(nnet_model_crf USING*)
ORDER BY 1,2;
...
FROM (SELECT DECODE(DEFAULTPAYNXTMNT, PREDICTION(nnet_model_crf using *), 1, 0)AS CORRECT
     FROM crf_mining_data_test_v);
