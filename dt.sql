...
DROP TABLE dt_model_crf_settings CASCADE CONSTRAINTS PURGE;

CREATE TABLE dt_model_crf_settings(
...
BEGIN
    insert into dt_model_crf_settings VALUES
...
BEGIN
  DBMS_DATA_MINING.DROP_MODEL(model_name => 'dt_model_crf');
END;
/
...
SELECT DEFAULTPAYNXTMNT AS Actuals,
    PREDICTION(dt_model_crf USING*) AS Predicted,
    COUNT(*) AS TOTALS
FROM crf_mining_data_test_v
GROUP BY DEFAULTPAYNXTMNT, PREDICTION(dt_model_crf USING*)
ORDER BY 1,2;
...
FROM (SELECT DECODE(DEFAULTPAYNXTMNT, PREDICTION(dt_model_crf using *), 1, 0)AS CORRECT
     FROM crf_mining_data_test_v);
